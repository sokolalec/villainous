...
object PlayerRecord {
  ...
  def numGamesGoingFirst(player: Player, games: Seq[DuelGame]): Int = {
    val firstLose = games.count(g => g.loserPlayer == player && g.loser == g.firstPlayer)
    val firstWin = games.count(g => g.winnerPlayer == player && g.winner == g.firstPlayer)
    firstLose + firstWin
  }
