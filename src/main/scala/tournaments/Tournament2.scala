    ...
    if (finished) {
      ...
      games().lastOption match {
        case None =>
          ...
          Some(PlayableDuel(player1, first, player2, last, getFirstPlayer(first, last)))
        case Some(g) =>
          ...
          nextTop.flatMap(nt => nextBottom.map(nb => {
            if (nt == nb) {
              if (winCount(topPlayer()) == ordering.size - 1) {
                ...
                PlayableDuel(topPlayer(), nt, bottom, bottomVillain, getFirstPlayer(nt, bottomVillain))
              } else if (winCount(bottom) == ordering.size - 1) {
                val topVillain = getNextVillain(topPlayer(), skipOne = true).get
                PlayableDuel(topPlayer(), topVillain, bottom, nb, getFirstPlayer(topVillain, nb))
              } else {
                ...
                PlayableDuel(topPlayer(), topVillain, bottom, bottomVillain, getFirstPlayer(topVillain, bottomVillain))
              }
            } else PlayableDuel(topPlayer(), nt, bottom, nb, getFirstPlayer(nt, nb))
          }))
